-*- mode: Org; mode: no-flyspell; comment-start: ";; " -*- 
...
#+begin_src emacs-lisp
(defun lva-select-random-drills ()
  (interactive)
  (let ((case-fold-search t) 
	(drill-heading (concat "* " "Quick drills"))
	list-start list-end drills)
    
    (save-excursion
      (goto-char (point-min))
      (search-forward drill-heading nil nil)
      (forward-line)
      (setq list-start (point))
      (re-search-forward "^*")
      (re-search-backward "[^\s-]")
      (setq list-end (1+ (point)))
      (setq drills (split-string 
	     (buffer-substring list-start list-end)
	     "\n"))
      (message (nth (random (length drills)) drills)))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (unless (eq system-type 'windows-nt)
    ...
    (setq org-file-apps (cons '("\\.xoj\\'" . "xournal %s") org-file-apps))
    (setq org-file-apps (cons '("\\.nb\\'" . "/opt/Wolfram/Mathematica/8.0/Executables/Mathematica %s") org-file-apps)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (define-key evil-normal-state-map "\M-k" 'evil-scroll-line-up)
  (define-key evil-normal-state-map "\M-j" 'evil-scroll-line-down)
...
** Nav
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elisp/nav/")
  (require 'nav)
#+end_src  
** Speedbar
#+begin_src emacs-lisp
...
;; makes emacs hang w/ latest 24.1.50.1 snapshot (20120807)
...
#+begin_src emacs-lisp 
  ...
  (eval-after-load 'scheme
    '(progn 
       (load-file "~/.emacs.d/elisp/scheme-complete-0.8.7.el")
...
Julia!
#+begin_src emacs-lisp
  ;; (require 'julia-mode) ;; trying the ESS version
  (setq auto-mode-alist (append '(("\\.jl$" . julia-mode)) auto-mode-alist))
  (add-hook 'julia-mode-hook (lambda () (setq indent-line-function 'julia-indent-line)))
#+end_src
Coffee!
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elisp/coffee-mode.git")
  (require 'coffee-mode)
#+end_src
** Webdev
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; Need this to properly indent @Override statements:
...
(if (not (eq emacs-profile 'linux-default))
    (require 'ess-site))
(setq inferior-julia-program-name "~/bin-local/julia-release-basic")
...
#+begin_src emacs-lisp
  ...
  (if (boundp 'inferior-ess-mode-map)
